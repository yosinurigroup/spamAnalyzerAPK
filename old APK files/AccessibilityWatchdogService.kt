...
class AccessibilityWatchdogService : Service() {
    ...
    override fun onCreate() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            // ✅ Required on Android 13+ (incl. 14/15)
            registerReceiver(
                stateReceiver,
                filter,
    ...
    private fun pokeAssist() {
        // app-internal broadcast ko implicit na rakhein — aap ne already setPackage use kiya hua hai 👍
        sendBroadcast(
            Intent(AssistCaptureService.ACTION_REFRESH_KEYWORDS)
                .setPackage(packageName) // ✅ keep it explicit
        )
        // companions ko alive rakho
        try {
